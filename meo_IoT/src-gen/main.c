...
Retcode_T Mita_initialize(void* userParameter1, uint32_t userParameter2)
{
	...
	exception = ConnectivityHttpRestClientServer_Setup();
	if(exception == NO_EXCEPTION)
	{
		printf("[INFO, %s:%d] setup ConnectivityHttpRestClientServer succeeded\n", __FILE__, __LINE__);
	}
	else
	{
		printf("[ERROR, %s:%d] failed to setup ConnectivityHttpRestClientServer\n", __FILE__, __LINE__);
		return exception;
	}
...
Retcode_T Mita_goLive(void* userParameter1, uint32_t userParameter2)
{
	...
	exception = ConnectivityHttpRestClientServer_Enable();
	if(exception == NO_EXCEPTION)
	{
		printf("[INFO, %s:%d] enable ConnectivityHttpRestClientServer succeeded\n", __FILE__, __LINE__);
	}
	else
	{
		printf("[ERROR, %s:%d] failed to enable ConnectivityHttpRestClientServer\n", __FILE__, __LINE__);
		return exception;
	}
